...
本次评测中，我们将使用一个大规模情感词典

[^1]: 徐琳宏, 林鸿飞, 潘宇,等. 情感词汇本体的构造[J]. 情报学报, 2008, 27(2):180-185.

，过滤掉所有包含显式情感词的文本。对这类不含显式情感词的数据进行标注，将数据标注为：褒义隐式情感、贬义隐式情感以及不含情感倾向的句子。评测数据以切分句子的篇章形式发布，保留了完整的上下文内容信息。
...
加粗为标记句子，含有完整的上下文，标签为：0-不含情感，1-褒义隐式情感，2-贬义隐式情感。
...
本次阶段由[国家自然科学基金重点项目——社交媒体中文本情感语义计算理论和方法](http://sa-nsfc.com/)**在线评测平台**(http://sa-nsfc.com/evaluation/)提供支持。参赛队伍需登录该平台注册报名[SMP2019“拓尔思杯”中文隐式情感分析评测（SMP-ECISA 2019）](http://sa-nsfc.com/evaluation/ecisa/scheme)获取数据集。其中训练集由SMP2019-ECISA的训练、验证集构成，验证集由SMP2019-ECISA的测试集构成。
...
**本阶段结果提交方式：**参赛队伍需要将包含结果文件的邮件发送至评测官方邮箱sxu_nlp@163.com，邮件的标题为“SMP2021-参赛队名-提交次数”，邮件附件为结果文件。
...
联系人：廖健　　联系方式：[liaojian_reg@163.com](liaojian_reg@163.com)
